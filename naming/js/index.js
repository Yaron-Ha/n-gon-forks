        ...
        case input.key.pause:
            if (!simulation.isChoosing && input.isPauseKeyReady && m.alive) {
                ...
                if (simulation.paused) {
                    ...
                    name.textContent = descript.textContent = '' // hide it
...
function cycle() {
    ...
    if (elapsed > simulation.fpsInterval) { // if enough time has elapsed, draw the next frame
        ...
        if (simulation.paused) {
            // pause loop. Uses plain HTML elements since setting up a second canvas (so erasing the
            // frame won't erase the simulation render) is super hard
            // we don't have access to simulation.mouseInGame since it's only updated on the regular loop
            const mouseInGameX = (simulation.mouse.x - canvas.width2) / simulation.zoom * simulation.edgeZoomOutSmooth + canvas.width2 - m.transX;
            const mouseInGameY = (simulation.mouse.y - canvas.height2) / simulation.zoom * simulation.edgeZoomOutSmooth + canvas.height2 - m.transY;
            const mouse = {x: mouseInGameX, y: mouseInGameY}
            // iterate over all game objects and check if they touch the mouse
            for (let entityGroup of [mob, [player], map, bullet, powerUp]) {
                const collisions = Matter.Query.point(entityGroup, mouse)
                // this really shouldn't be more than 1
                const hit = collisions[0]
                if (!hit) continue
                if (hit.name) {
                    console.log(hit.name)
                    // check if that game object has a name. If so,
                    // move the name shower there
                    name.textContent = hit.name
                    // nameShower.display = 'inline' // make it show up
                    console.log(nameShower.style.display)
                    nameShower.style.left = mouse.x + 'px'
                    nameShower.style.top = mouse.y + 'px'
                    console.log(nameShower.style)
