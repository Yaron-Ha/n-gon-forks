...
const spawn = {
    nonCollideBossList: ["energyBoss"],
    // other bosses: suckerBoss, laserBoss, tetherBoss,    //these need a particular level to work so they are not included in the random pool
    randomLevelBoss(x, y, options = ["energyBoss"]) {
        options = ["energyBoss"]
    ...
    energyBoss(x, y) {
        ...
        mobs.spawn(x, y, 9, RADIUS, "#0066CC")
        const me = mob[mob.length - 1]

        me.memory = 100 // fairly weak memory
        me.seeAtDistance2 = Math.pow(RADIUS * 15, 2)
        me.isBoss = true
        ...
        function spawnOrb(type) {
            const direction = simulation.randomUnitVector()
            // spawn an orb in that direction
            spawn.energyBossOrb(type, RADIUS, me.position, direction, orbs)
            orbs.push(mob[mob.length - 1])
        }
        ...
        me.do = function() {
            ...
            const recall = this.seePlayer.recall

            if (recall) this.attraction()
            // small chance to spawn an orb, the chances increases if the boss spots the player
            if (recall && Math.random() > 0.98 || Math.random() > 0.995) {
                // it is determined to spawn an orb. Now decide the type
                const chance = Math.random()
                if (chance > 0.95 && recall) {
                    spawnOrb(3) // spawn a killer orb
                } else if (chance > 0.8 && recall || chance > 0.95) {
                    // select splitter or killer
                    if (Math.random() > 0.5) {
                        spawnOrb(1)
                    } else {
                        spawnOrb(2)
            ...
            // slowly rotate, it looks pretty interesting
            Matter.Body.rotate(this, 0.01)
        }

        me.onDeath = function() {
            // kill all orbs on death, using foreach because this function is only called once,
            // so performance isn't a huge deal. I miss Kotlin... In Kotlin you can freely
            // use foreach on arrays and everything is inlined down to regular array iteration.
            powerUps.spawnBossPowerUp(this.position.x, this.position.y)

            for (const orb of orbs) {
                // orb.death() didn't work, no idea why
                orb.alive = false
            }
        }
    },
    energyBossOrb(type, parentRadius, parentPosition, direction, orbs) {
        /*
        The orb has four types.
        0: regular orb: randomly speeds across the screen, has green-ish color.
        1: explosive orb: more rare, will explode if it touches the player. Red color.
        2: cloner orb: rare as explosive orb, but has a chance to clone itself, with a high chance of becoming a killer orb. Purple color.
        3: killer orb: rarest type, has more health and activily tries to smash into the player. Brownish-black.
        All orbs except the first one are very rare if the player isn't nearby.
        The killer orb won't spawn at all if the player isn't close.
        */
        // parentRadius is supposed to be used in a calculation that prevents the orb from being
        // spawned inside the parent boundaries.
        function determineColor() {
            switch (type) {
                case 0: return `rgb(20, 255, ${~~(70 + Math.random() * 40)})`
                case 1: return `rgb(255, 20, ${~~(20 + Math.random() * 40)})`
                case 2: return `hsl(304, 100%, ${~~(30 + Math.random() * 30)}%)`
                case 3: return `rgb(${~~(10 + Math.random() * 50)}, 0, 0)`
                default: return null
            }
        }

        const MAXTICK  = 1000 // the maximum time of which the orb can live
        const RADII    = [20, 30, 25, 15] // the radii of the different types, accessed by the type as index
        const MAXSPEED = 20
        // scale the parent location with the direction to spawn the mob at the right place... except it doesnt work
        // const combinedRadii = parentRadius + BASERADIUS
        const location = Vector.add(parentPosition, direction)

        mobs.spawn(location.x, location.y, 12, RADII[type], determineColor())
        const me = mob[mob.length - 1]
        me.friction = 0 // no friction
        me.restitution = 1

        // give it the speed
        Matter.Body.setVelocity(me, Vector.mult(direction, Math.random() * MAXSPEED))

        me.tick = 0
        // the life must always be higher than 1/3 of Max tick, but lower than maxtick
        me.life = Math.min(MAXTICK * Math.random() + MAXTICK / 3, MAXTICK)
        me.isDropPowerUp = false // obviously drop no powerups, that would mean a ton of camping
        me.leaveBody = false
        me.showHealthBar = false

        me.do = function() {
            if (type === 2 && orbs.length < 20 && Math.random() < 0.05 / orbs.length) {
                // this is a cloner, so it'll clone itself, but the clone chance goes down the more mobs there are
                if (Math.random() > 0.8) {
                    // 20% chance for the clone to become a killer
                    spawn.energyBossOrb(3, RADII[type], this.position, simulation.randomUnitVector(), orbs)
                } else {
                    spawn.energyBossOrb(2, RADII[type], this.position, simulation.randomUnitVector(), orbs)
                }
                orbs.push(mob[mob.length - 1])
                // cool graphic to let the player know a clone has happened
                simulation.drawList.push({
                    x: this.position.x,
                    y: this.position.y,
                    radius: this.radius * 2 + Math.random() * this.radius,
                    color: "#ab5cff",
                    time: 10
                })
                simulation.drawList.push({
                    x: this.position.x,
                    y: this.position.y,
                    radius: this.radius * 4 + Math.random() * this.radius * 2,
                    color: "#ff5cf4",
                    time: 14
                })
            }

            // low chance to give itself a boost, which becomes higher if this is a killer orb.
            // if the speed is very low, it'll give a boost regardless
            if (
                ((type === 3 && Math.random() > 0.95) || // it's a killer orb
                (this.speed < 1) || // it's very slow
                (Math.random() > 0.99) // otherwise, low chance
                && !me.isBoosting) && this.speed < MAXSPEED) // it's not boosting and not too fast
            {
                me.isBoosting = true
                me.boostTick = 0
            }
            ...
            if (me.isBoosting) {
                ...
                const rotation = Math.random() > 0.5 ? 0.01 : -0.01
                if (type === 3) {
                    // if it's a killer orb, boost into the player if it can see it
                    this.seePlayerCheck()
                    this.checkStatus()
                    if (this.seePlayer.recall) {
                        Matter.Body.setVelocity(this, Vector.mult(Vector.normalise(Vector.sub(player.position, this.position)), this.speed * 1.02))
                    }
                } else {
                    // it's not, boost forward with a *tiny* rotation
                    Matter.Body.setVelocity(this, Vector.rotate(Vector.mult(this.velocity, 1.03), rotation))
                }
                
                simulation.drawList.push({
                    x: this.position.x,
                    y: this.position.y,
                    radius: this.radius / 2 + me.boostTick / 4,
                    color: `hsl(${me.boostTick}, 100%, 50%)`,
                    time: 3
                })
            ...
            const remainingTicks = this.life - this.tick
            if (remainingTicks < 50) {
                // not much life left, so scale down instead of just disappearing
                Matter.Body.scale(this, 0.98, 0.98)
                // die if the life is exceeded
                if (remainingTicks < 0) this.death()
            }
        ...
        me.onHit = function() {
            // whenever an orb hits a player, its life is elongated
            this.life *= 2
            if (type === 1) {
                // hit or take damage = explode
                // this is an explosive orb, so explode with thrice the mass!
                this.explode(this.mass * 3)
            }
        }
        ...
        me.onDeath = function() {
            if (type === 1) {
                // the explosive orbs release 3-7 bullets on death
                for (let i = 0; i < ~~(3 + Math.random() * 5); i++) {
                    spawn.bullet(this.position.x, this.position.y, RADII[type] / 3, 3)
                    const randVector = Vector.mult(simulation.randomUnitVector(), Math.random() * 30)
                    Matter.Body.setVelocity(mob[mob.length - 1], randVector)
                }
                // also create a small explosion graphic because why not
                simulation.drawList.push({
                    x: this.position.x,
                    y: this.position.y,
                    radius: this.radius * 2 + Math.random() * this.radius,
            ...
            // removes itself from orbs to prevent a memory leak
            // I thought about doing WeakRef instead, which worked but obviously didn't change the length of the array
