const tech = {
    ...
    tech: [{
        ...
        {
            name: "nuclear mutation",
            description: "<strong class='color-p'>irradiated</strong> <strong>grenade</strong> explosions increase <strong class='color-d'>damage</strong>, but mutate mobs and add a chance for them to increase <strong class='color-harm'>harm</strong>.",
            isGunTech: true,
            maxCount: 9,
            count: 0,
            frequency: 2,
            frequencyDefault: 2,
            allowed() {
                return tech.haveGunCheck("grenades") && !tech.fragments && !tech.isVacuumBomb && !tech.isExplodeRadio && tech.isNeutronBomb
            },
            requires: "grenades, not fragmentation, vacuum bomb, iridium-192",
            effect() {
                if (!tech.mutationLevel) tech.mutationLevel = 0
                tech.mutationLevel += 1
                // b.setGrenadeMode()
            },
            remove() {
                tech.mutationLevel = null
                // b.setGrenadeMode()
            }
        },
        {
