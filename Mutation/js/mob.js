...
const mobs = {
    ...
    statusDoT(who, tickDamage, cycles = 180, mutationLevel = 0) {
        // some radiation sources introduce mutation
        if (!who.isShielded && who.alive && who.damageReduction > 0) {
            who.status.push({
                effect() {
                    ...
                    // do mutation effect
                    if (mutationLevel) { // if there's any mutation chance
                        const vtx = who.vertices
                        // fairly small chance
                        for (let i = 0; i < vtx.length; i++) {
                            if (Math.random() * 10 + mutationLevel > 9.5) {
                                // pick a random point near the vertex
                                const newPoint = 
                                    Vector.add(vtx[i], Vector.create(Math.random() - 0.5, Math.random() - 0.5))
                                // now make sure that that point keeps the polygon convex, using the point after and the point before
