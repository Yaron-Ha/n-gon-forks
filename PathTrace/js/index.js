...
// the space between the nodes. The higher it is, the more accurate the algorithm,
// and the slower it is, because more nodes need to be traversed. Nodes use the regular
// coordinate system, except divided by SPACE. To avoid confusion they'll be prefixed by 'm'
const SPACE = 25

const nodes = {
    // caches all nodes for the map using a spatious array
    nodeCache: [],
    getNode(mx, my) {
        // returns a node in specific location, or null if it collides with the map.
        // since the map never changes, all calculations are lazily calcualted and cached
        // Cantor's pairing function I stole from the internet and serves as a nice key "hash" for the array
        const hash = ((mx + my) * (mx + my + 1) / 2) + my
        const tryCached = this.nodeCache[hash]
        // we already calculated this! Return the old node
        if (tryCached !== undefined) return tryCached
        // we never calculated this node before, so let's do it
        if (Matter.Query.point(map, { x: mx * SPACE, y: my * SPACE }).length === 0) {
            // great, no collision between the map and this node. Cache and return it
            const node = { mx, my }
            this.nodeCache[hash] = node
            return node
        } else {
            // null means that this node was calculated, but is invalid (inside a block). Cache null and return it
            this.nodeCache[hash] = null
            return null
        }
    },

    getSiblings(node) {
        // takes a single node and returns 4 sibling nodes, one from each side
        // UP, RIGHT, DOWN, LEFT
        const { mx, my } = node
        return [this.getNode(mx, my - 1),
                this.getNode(mx + 1, my),
                this.getNode(mx, my + 1),
                this.getNode(mx - 1, my)]
    },
    testList: [],
    test() {
        const recursive = (node, depth) => {
            if (depth === 0) return
            if (this.testList.find(it => it.mx == node.mx && it.my == node.my)) {
                // if it's already visited, return
                return
            } else {
                this.testList.push(node)
            }
            const siblings = this.getSiblings(node)
            for (const member of siblings) {
                if (member) {
                    // if it's not null, continue in that direction
                    recursive(member, depth - 1)
                }
            }
        }
        let location = { mx: ~~(player.position.x / SPACE), my: ~~(player.position.y / SPACE) }
        recursive(location, 1000)
    }
}

const pathfind = {
    pathFind(source, dest, objectRadius) {
        // heuristic:
        // if the source and destination can see each other, return null.
        // technically there is a small path to traverse but the objects can just push
        // themselves to reach the destination
        if (Matter.Query.ray(source, dest, objectRadius).length !== 0) return null
        // pathfind using a reverse priority queue
        const frontier = new PriorityQueue((a, b) => a[1] < b[1])

        const sourceNode = { mx: ~~(source.x / SPACE), my: ~~(source.y / SPACE), cameFrom: null }
        const destNode = { mx: ~~(dest.x / SPACE), my: ~~(dest.y / SPACE) }

        let result = null
        frontier.push([sourceNode, 0])
        
        while (!frontier.isEmpty()) {
            // take the most promising route
            const currentNode = frontier.pop()
            // if we reached the end, great!
            if (currentNode.mx == destNode.mx && currentNode.my == destNode.my) {
                result = currentNode
                break
            }
            for (const sibling of nodes.getSiblings(currentNode[0])) {
                // if it's a valid path through path, add it as well
                if (sibling) {
                    const manhattan = Math.abs(sourceNode.mx - sibling.mx) + Math.abs(sourceNode.my - sibling.my)
                    const newCost = currentNode[1] + manhattan
                    sibling.cameFrom = currentNode
                    frontier.push([sibling, newCost])
                }
            }
        }
        return result
