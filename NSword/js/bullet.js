...
const b = {
    ...
    sword() {
        const DIST_FROM_PLAYER = 50
        const BASE_WIDTH = 10
        const BASE_HEIGHT = 200
        const PHYSICAL_DIST = DIST_FROM_PLAYER + BASE_HEIGHT / 2
        const me = Bodies.rectangle(m.pos.x + PHYSICAL_DIST * Math.cos(m.angle), m.pos.y + PHYSICAL_DIST * Math.sin(m.angle), BASE_HEIGHT * tech.bulletSize, BASE_WIDTH * tech.bulletSize, b.fireAttributes(m.angle))
        bullet.push(me)

        Matter.Body.setDensity(me, 0.00001) //0.001 is normal
        me.dmg = 6
        me.endCycle = Infinity
        me.collisionFilter.mask = cat.body | cat.mob | cat.map | cat.powerUp
        ...
        me.do = function() {
            ...
            //     this.force = Vector.mult(Vector.normalise(Vector.sub(player.position, this.position)), 0.0001)
            ...
            if (input.fire) {
                // set position
                // Matter.Body.setPosition(this, {x: m.pos.x + PHYSICAL_DIST * Math.cos(m.angle), y: m.pos.y + PHYSICAL_DIST * Math.sin(m.angle)})
                // set angle
        ...
        Composite.add(engine.world, me) //add bullet to world
    },
    // **************************************************************************************************
    // **************************************************************************************************
    // ********************************         Bots        *********************************************
    ...
    guns: [{
        ...
        {
			name: 'sword',
			description: 'a strong <strong>melee</strong> weapon, does not require ammo.',
			ammo: 0,
			ammoPack: Infinity,
			have: false,
            // there is only one sword at any given moment!
            swordInstance: null,
            // if the last fired tick is more than a threshold, the sword fades
            // until it gets used next time
            lastFiredTick: 0,
			fire() {
                // first spawn a sword if there isn't one
                if (!this.swordInstance) {
                    b.sword()
                    this.swordInstance = bullet[bullet.length - 1]
                }
            },
			do() {
                const FADE_TICK = 100
                if (++this.lastFiredTick > FADE_TICK) {
                    // fade
                }
            },
			chooseFireMethod() {
