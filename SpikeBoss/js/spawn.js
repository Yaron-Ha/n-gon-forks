...
const spawn = {
    ...
    //     };
    // },
    spiker(x, y, radius = 50) {
        const BASE_COOLDOWN = 200 // delay between shots

        mobs.spawn(x, y, 8, radius, "#ffb3b3") // I love this buttery pink color
        const me = mob[mob.length - 1]
        // Cooldown times the delay between shots. Animation ticks manages a firing animation
        me.cooldown = me.animationTicks = 0
        me.do = function() {
            this.seePlayerByLookingAt()
            this.checkStatus()
            this.attraction()
            ...
            if (this.cooldown < 0 && Math.random() > 0.95) {
                // the cooldown is reduced the harder the game is
                this.cooldown = Math.floor(BASE_COOLDOWN * 4 / simulation.difficulty)
                // add a cool animation
                this.animationTicks = 100
                // shoot
            }

            if (this.animationTicks > 0) {
                ctx.fillStyle = `rgba(255, 20, 20, ${this.animationTicks / 10})`
                ctx.lineWidth = 3
                // ctx.arc()
                const v = this.vertices // make it short
                for (let i = 0; i < v.length; i++) {
                    ctx.beginPath()
                    ctx.moveTo(v[i].x, v[i].y) // move to the selected vertex
                    const otherIndex = (i + 3) % v.length // select another index in + 3 (circular)
                    ctx.lineTo(v[otherIndex].x, v[otherIndex].y)
                    ctx.fill()
                }
                this.animationTicks--
            }

            // every tick, reduce the cooldown. I've decided to make is possibly negative
            // by not doing a cooldown > 0 check to eliminate extra branching
            this.cooldown--
        }
    },
