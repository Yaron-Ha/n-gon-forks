...
const level = {
	...
	start() {
		...
		if (tech.isForeverDrones) {
			if (tech.isDroneRadioactive) {
				for (let i = 0; i < tech.isForeverDrones * 0.25; i++) {
					b.droneRadioactive(
						{ x: m.pos.x + 30 * (Math.random() - 0.5), y: m.pos.y + 30 * (Math.random() - 0.5) },
						5
					)
		...
		if (tech.isSwitchReality && powerUps.research.count > 0) {
			...
			simulation.trails()
			powerUps.spawn(
				player.position.x + Math.random() * 50,
				player.position.y - Math.random() * 50,
				'tech',
				false
			)
		}
		if (tech.isHealLowHealth) {
			...
			for (let i = 0; i < len; i++)
				powerUps.spawn(
					player.position.x + 90 * (Math.random() - 0.5),
					player.position.y + 90 * (Math.random() - 0.5),
					'heal',
					false
				)
		}
		...
		if (tech.isSpawnExitTech) {
			for (let i = 0; i < 2; i++)
				powerUps.spawn(
					level.exit.x + 10 * (Math.random() - 0.5),
					level.exit.y - 100 + 10 * (Math.random() - 0.5),
					'tech',
					false
				) //exit
	...
	trainingText(say) {
		...
		simulation.makeTextLog(
			`<span style="font-size: 120%;line-height: 120%;"><span style="color:#51f;">supervised.learning</span>(<span style="color:#777; font-size: 80%;">${(
				Date.now() / 1000
			).toFixed(0)} s</span>)<span class='color-symbol'>:</span><br>${say}</span>`,
			Infinity
		)
	...
	levelAnnounce() {
		...
		if (level.levelsCleared === 0) {
			...
			simulation.makeTextLog(
				`<span class='color-var'>level</span>.onLevel <span class='color-symbol'>=</span> "<span class='color-text'>${
	...
	populateLevels() {
		...
		if (simulation.isTraining) {
			...
			level.levels.splice(
				Math.floor(Math.seededRandom(level.levels.length * 0.6, level.levels.length)),
				0,
				'reservoir'
			) //add level to the back half of the randomized levels list
			level.levels.splice(
				Math.floor(Math.seededRandom(level.levels.length * 0.6, level.levels.length)),
				0,
				'reactor'
		...
		for (let i = 0; i < level.levels.length; i++)
			spawn.mobTypeSpawnOrder.push(
				spawn.fullPickList[Math.floor(Math.seededRandom(0, spawn.fullPickList.length))]
			)
		spawn.bossTypeSpawnOrder = []
		for (let i = 0; i < level.levels.length * 2; i++)
			spawn.bossTypeSpawnOrder.push(
				spawn.randomBossList[Math.floor(Math.seededRandom(0, spawn.randomBossList.length))]
			)
	},
	flipHorizontal() {
		const flipX = who => {
	...
	addToWorld() {
		...
		for (let i = 0; i < map.length; i++) {
			...
			map[i].collisionFilter.mask =
				cat.player | cat.map | cat.body | cat.bullet | cat.powerUp | cat.mob | cat.mobBullet
	...
	boost(x, y, height = 1000) {
		...
		who = map[map.length] = Matter.Bodies.fromVertices(
			x + 50,
			y + 35,
			Vertices.fromPath('120 40 -120 40 -50 -40 50 -40'),
			{
				collisionFilter: {
					category: cat.body,
			...
			query() {
				if (Matter.Query.region(body, this).length === 0 && Matter.Query.region([player], this).length === 0) {
					...
					if (this.isUp === true) {
						...
						if (list.length > 0) {
							if (
								list[0].bounds.max.x - list[0].bounds.min.x < 150 &&
								list[0].bounds.max.y - list[0].bounds.min.y < 150
							) {
					...
					if (this.isTouched) {
						if (!m.isBodiesAsleep) {
							...
							Matter.Body.setVertices(
								this,
								this.shrinkVertices(Math.max(this.fadeCount / this.fadeTime, 0.03))
							)
						}
						if (this.fadeCount < 1) {
						...
						if (this.returnCount < 1) {
							...
							if (Matter.Query.collides(this, [player]).length) {
								...
								this.collisionFilter.mask =
									cat.player | cat.mob | cat.body | cat.bullet | cat.powerUp | cat.mobBullet
								this.fadeCount = this.fadeTime
	...
	portal(centerA, angleA, centerB, angleB) {
		...
		query = function (isRemoveBlocks = false) {
			if (Matter.Query.collides(this, [player]).length === 0) {
				...
				if (this.portalPair.angle % (Math.PI / 2)) {
					...
					if (m.immuneCycle < m.cycle + tech.collisionImmuneCycles)
		...
		const mapA = (composite[composite.length] = Bodies.rectangle(
			centerA.x - 0.5 * unitA.x * mapWidth,
			centerA.y - 0.5 * unitA.y * mapWidth,
			mapWidth,
			height + 10,
			{
				collisionFilter: {
		...
		const mapB = (composite[composite.length] = Bodies.rectangle(
			centerB.x - 0.5 * unitB.x * mapWidth,
			centerB.y - 0.5 * unitB.y * mapWidth,
			mapWidth,
			height + 10,
			{
				collisionFilter: {
			...
			query() {
				if (this.isOn) {
					...
					if (this.height > 0 && Matter.Query.region([player], this).length) {
						...
						const slowY =
							player.velocity.y > 0
								? Math.max(0.8, 1 - 0.002 * player.velocity.y * player.velocity.y)
						...
						const buoyancy =
							...
							(0.2 *
								Math.max(
									...
									Math.min(
										diameter,
										this.min.y - powerUpCollide[i].position.y + powerUpCollide[i].size
									)
								)) /
								diameter
		...
		exitOptions = [
			(x = offset.x, y = offset.y) => {
				...
				balance1 = level.spinner(
					...
					400,
					density,
					angle + variance * (Math.random() - 0.5),
					frictionAir,
					angularVelocity + spinVariance * (Math.random() - 0.5)
				) //    spinner(x, y, width, height, density = 0.001, angle=0,frictionAir=0.001,angularVelocity=0) {
				balance2 = level.spinner(
					x + 200,
					y - 950,
					30,
					400,
		...
		emptyOptions = [
			...
			(x = offset.x, y = offset.y) => {
				...
				if (!isDoorLeft && isDoorRight) {
					...
					for (let i = 0; i < 6; i++)
						spawn.bodyRect(x + 2000 - 60 - 555 + Math.floor(Math.random() * 10), y + -520 - 50 * i, 50, 50) //bullets for gun
					...
					doCustom.push(() => {
						...
						if (!button.isReadyToFire && button.isUp) {
							button.isReadyToFire = true
							bulletDoor.collisionFilter.mask =
								cat.player | cat.map | cat.body | cat.bullet | cat.mob | cat.mobBullet
						} else if (button.isReadyToFire && !button.isUp) {
							button.isReadyToFire = false
					...
					const bulletDoor = map[map.length - 1] //keep track of this body so it can be make non-collide later
					for (let i = 0; i < 6; i++)
						spawn.bodyRect(x + 555 + Math.floor(Math.random() * 10), y + -520 - 50 * i, 50, 50) //bullets for gun
				...
				doCustomTopLayer.push(() => {
					...
					if (button.isUp) {
						...
						if (!button.isUp) {
							...
							let r = 150
							let hexagon = `${r} 0   ${r * Math.cos(5.236)} ${r * Math.sin(5.236)}    ${
								r * Math.cos(4.189)
							} ${r * Math.sin(4.189)}     ${-r} 0     ${r * Math.cos(2.0944)} ${
								r * Math.sin(2.0944)
							}      ${r * Math.cos(1.0472)} ${r * Math.sin(1.0472)}  `
							//450 horizontal spread //  -130-130-130 = 390 vertical
							if (Math.random() < 0.5) {
								spawn.mapVertex(x + 775, y + -260, hexagon)
								spawn.mapVertex(x + 1225, y + -260, hexagon)
								...
								for (let i = 0; i < numberOfMapElementsAdded; i++)
				...
				doCustomTopLayer.push(() => {
					...
					if (button.isUp) {
						...
						if (!button.isUp) {
							...
							r = 150
							const hexagon150 = `${r} 0   ${r * Math.cos(5.236)} ${r * Math.sin(5.236)}    ${
								r * Math.cos(4.189)
							} ${r * Math.sin(4.189)}     ${-r} 0     ${r * Math.cos(2.0944)} ${
								r * Math.sin(2.0944)
							}      ${r * Math.cos(1.0472)} ${r * Math.sin(1.0472)}  `
							...
							//left side
							r = 350
							let hexagonHalf = `${r} 0   ${r * Math.cos(5.236)} ${r * Math.sin(5.236)}    ${
								r * Math.cos(4.189)
							} ${r * Math.sin(4.189)}     ${-r} 0 `
	...
	null() {
		...
		level.customTopLayer = () => {
			...
			for (let i = -19.5; i < 20; i++) {
				...
				ctx.bezierCurveTo(
					sway.x * Math.abs(i) + where.x,
					where.y + 25 * Math.abs(i) + 60 + sway.y * Math.sqrt(Math.abs(i)),
					sway.x * Math.abs(i) + where.x + horizontalStep * i,
					where.y + 25 * Math.abs(i) + 60 + sway.y * Math.sqrt(Math.abs(i)),
					horizontalStep * i,
					-800
				)
	...
	biohazard() {
		...
		level.custom = () => {
			...
			if (Math.random() > 0.995 && bossInit) {
				const spawnLocs = [-1415, -30, 1345, 2815, 4285]
				const randAngle = rndInRange(-Math.PI / 2, Math.PI / 2)
		...
		function energyBeacon(x, y, parentBoss) {
			...
			Matter.Body.setDensity(me, 0.0022 * 2 + 0.0002 * Math.sqrt(simulation.difficulty)) //extra dense
			me.do = function () {
		...
		function geneticSeeker(x, y, genome, parentBoss) {
			...
			mobs.spawn(
				...
				genome.vertexCount,
				genome.size,
				'#' + genome.color.map(it => (~~it).toString(16)).join('')
			)
			const me = mob[mob.length - 1]
			// apply genome
			...
			me.onHit = function () {
				...
				this.score += 50
				this.score += this.mass // bigger mass = bigger damage, add that too
				// a marker for later
				this.hitPlayer = true
		...
		function geneticBoss(x, y, radius = 130, spawnBossPowerUp = true) {
			...
			const MAX_BEACONS = Math.round(Math.random() * simulation.difficulty + 3)
			...
			me.fittestOrbs = localSettings.genome ?? [init(), init(), init()] // best genomes so far. Size of three
			...
			me.onDeath = function () {
				...
				localSettings.genome = this.fittestOrbs
			...
			me.do = function () {
				...
				this.harmZone()
				// 
				const regularChance = Math.random() > 0.98
				const biggerChance = Math.random() > 0.93 && this.energy > 0.25
			...
			const amount = ~~(5 + Math.random() * simulation.difficulty / 2)
			for (let i = 0; i < amount; i++)
				me.spawnBeacon()
		}
	...
	//     for (let i = 0; i < 5; i ++) b.giveGuns("random", 1000)

	//     tech.giveTech("necrophage") // for testing
	...
	//         }
	//         // initialized here as it depends on vertexCount. I could use `new function()` but nah.
	//         genome.vertexOffset = Array(genome.vertexCount).fill().map(() => ({x: Math.random() - 0.5, y: Math.random() - 0.5}))
	//         return genome
	//     }
	...
	//         // transports energy to the boss
	//         // when the boss is hit by it, how much of the energy stored the boss actually recives
	//         const ENERGY_TRANSFER_RATE = 30/*%*/

	//         const color = `rgba(${150 + 105 * charge}, 81, 50, 0.6)`
	//         mobs.spawn(origin.x, origin.y, 12, 20 + 20 * charge, color)
	//         const me = mob[mob.length - 1]
	//         me.end = function() {
	//             simulation.drawList.push({ // some nice graphics
	//                 x: this.position.x,
	//                 y: this.position.y,
	//                 radius: this.radius,
	//                 color: '#f3571d',
	...
	//             this.death()
	//         }
	//         me.collisionFilter.mask = cat.player | cat.map | cat.bullet
	//         // me.onHit = this.end
	//         me.life = 0
	//         me.isDropPowerUp = false
	//         me.leaveBody = false
	...
	//                 this.end()
	//             }
	//             // if the player collides with the boss, die but give energy to the boss
	//             if (Matter.Query.collides(this, [boss]).length > 0) {
	//                 boss.energy = Math.min((charge * (ENERGY_TRANSFER_RATE / 100)) + boss.energy , 1)
	//                 this.end()
	//             }
	...
	//         me.chargeTicks = 0 // used to time charging the boss
	//         me.bossPos = null // the position that the mob remembers when charging
	//         Matter.Body.setDensity(me, 0.0022 + 0.0002 * Math.sqrt(simulation.difficulty)) //extra dense
	//         me.do = function() {
	//             // slowly rotate
	//             Matter.Body.setAngularVelocity(this, 0.01)
	//             this.fill = `rgba(${this.energy * 255}, 29, 136, 0.80)`
	...
	//                     ctx.lineTo(m.pos.x, m.pos.y)
	//                     ctx.lineWidth = 3 + Math.abs(Math.sin((simulation.cycle + this.seed) / 100)) * 2
	//                     ctx.strokeStyle = `rgb(${(Math.abs(Math.sin((simulation.cycle + this.seed + 100) / 100)) * 255).toFixed(3)}, 204, 255)`
	//                     ctx.setLineDash([125 * Math.random(), 125 * Math.random()]);
	//                     ctx.stroke()
	//                     ctx.setLineDash([])
	//                 }
	//             }
	//             // if the mob's energy is at least 50% full, try to send that energy to the boss.
	//             if (this.energy > 0.5 && parentBoss.energy < 1 && this.chargeTicks === 0) {
	//                 const seesBoss = Matter.Query.ray(map, this.position, parentBoss.position).length === 0
	//                 if (seesBoss) {
	//                     this.chargeTicks = 100
	//                     this.bossPos = Vector.clone(parentBoss.position)
	...
	//                 if (--this.chargeTicks === 0) {
	//                     // spawn the orb
	//                     const location = Vector.add(Vector.mult(Vector.normalise(Vector.sub(this.bossPos, this.position)), this.radius * 3), this.position)
	//                     energyTransferBall(location, this.bossPos, parentBoss, this.energy)
	//                     this.energy = 0
	//                 }
	//                 // create a beam and aim it at the bossPos
	...
	//                 ctx.setLineDash([])
	//             }
	//             // generate 0.15% energy per tick
	//             if (this.energy < 1) this.energy += 0.0015
	//             // draw energy bar
	//             drawEnergyBar(this)
	//         }
	//         me.onDeath = function() {
	...
	//             // explode with the strength of its energy!
	//             this.alive = false // to prevent retriggering infinitly
	//             b.explosion(this.position, this.energy * this.radius * 20)
	//         }
	//         // me.
	//     }
	//     function geneticSeeker(x, y, genome, parentBoss) {
	...
	//         // starts by random
	//         const init = () => ({genome: generateGenome(), score: 0})
	//         me.fittestOrbs = [init(), init(), init()] // best genomes so far. Size of three
	//         // when an orb died it's moved here. When a new spawn cycle starts, their scores get calculated
	//         // and they get put in the fittest orbs array, if they are better than the old ones.
	//         me.deadOrbs = []
	//         me.energy = 0
	//         // this boss has no orbitals, because it's not meant to ever attack on its own.
	//         // that is to balance its invincibility.
	...
	//             this.attraction()
	//             this.repulsion()
	//             // spawn some energy beacons if there are less than the maximum.
	//             // small chance if there's no energy, bigger chance if there is at least 25% (which is drained)
	//             const regularChance = Math.random() > 0.995
	//             const biggerChance = Math.random() > 0.99 && this.energy > 0.25
	//             if (this.energyBeacons.length < MAX_BEACONS && biggerChance || regularChance) {
	//                 if (biggerChance) {
	//                     // if the spawn was a selection of bigger chance, reduce 25% energy
	//                     this.energy -= 0.25
	//                 }
	//                 // the vertex to spawn the beacon from
	//                 const vert = this.vertices[~~(Math.random() * this.vertices.length)]
	...
	//                         // the evolution part comes here, as the genome is mutated first.
	//                         let randGenome = mutateGenome(bestOrbs[~~(Math.random() * bestOrbs.length)])
	//                         const cost = calculateGenomeCost(randGenome)
	//                         if (this.energy - cost < 0) {
	//                             // okay this orb is too expensive for the boss to spawn,
	//                             // make a new orb from scratch
	//                             randGenome = generateGenome()
	...
	//                         geneticSeeker(vertex.x, vertex.y, randGenome, this)
	//                         // give the bullet a rotational velocity as if they were attached to a vertex
	//                         const velocity = Vector.mult(Vector.perp(Vector.normalise(Vector.sub(this.position, vertex))), -10)
	//                         Matter.Body.setVelocity(mob[mob.length - 1], {
	//                             x: this.velocity.x + velocity.x,
	//                             y: this.velocity.y + velocity.y
	//                         })
	...
	//                 }
	//             }
	//             if (this.energy > 1) this.energy -= 0.003 // clean excess energy
	//             // draw supershield
	...
	reactor() {
		...
		spawn.mapVertex(
			1487,
			-900,
			`${-a} ${-a + c}  ${-a + c} ${-a}   ${a - c} ${-a}  ${a} ${-a + c}   ${a} ${a - c}  ${a - c} ${a}  ${
				-a + c
			} ${a}  ${-a} ${a - c}`
		) //square with edges cut off
		//entrance
					...
					if (player.position.x < 0) {
						if (!doorOut.isClosed() || !doorIn.isClosed()) {
							...
							if (Math.random() < 0.07 && simulation.difficulty > 24) {
								for (let i = 0, len = (scale * 0.25) / 4; i < len; ++i)
									spawn.timeBoss(-1327 - 200 * i, -1525, 60, false) //spawn 1-2 at difficulty 15
								for (let i = 0, len = (scale * 0.1) / 4; i < len; ++i)
									spawn.bounceBoss(-1327 - 200 * i, -1525, 80, false)
								for (let i = 0, len = (scale * 0.16) / 4; i < len; ++i)
									spawn.sprayBoss(-1327 - 200 * i, -1525, 30, false)
								for (let i = 0, len = (scale * 0.23) / 4; i < len; ++i)
									spawn.mineBoss(-1327 - 200 * i, -1525, 50, false)
							} else {
								if (Math.random() < 0.25) {
									for (let i = 0, len = scale * 0.25; i < len; ++i)
										spawn.timeBoss(-1327 - 200 * i, -1525, 80, false) //spawn 1-2 at difficulty 15
								} else if (Math.random() < 0.33) {
									for (let i = 0, len = scale * 0.1; i < len; ++i)
										spawn.bounceBoss(-1327 - 200 * i, -1525, 80, false) //spawn 1-2 at difficulty 15
								} else if (Math.random() < 0.5) {
									for (let i = 0, len = scale * 0.16; i < len; ++i)
										spawn.sprayBoss(-1327 - 200 * i, -1525, 30, false) //spawn 2-3 at difficulty 15
								} else {
									for (let i = 0, len = scale * 0.23; i < len; ++i)
										spawn.mineBoss(-1327 - 200 * i, -1525, 50, false) //spawn 3-4 at difficulty 15
								}
							}
				...
				if (button.isUp) {
					...
					if (player.position.x > 0) {
						if (!doorOut.isClosed() || !doorIn.isClosed()) {
							...
							if (Math.random() < 0.07 && simulation.difficulty > 24) {
								for (let i = 0, len = (scale * 0.25) / 4; i < len; ++i)
									spawn.timeBoss(1487 + 200 * i, -1525, 60, false) //spawn 1-2 at difficulty 15
								for (let i = 0, len = (scale * 0.1) / 4; i < len; ++i)
									spawn.bounceBoss(1487 + 200 * i, -1525, 80, false)
								for (let i = 0, len = (scale * 0.16) / 4; i < len; ++i)
									spawn.sprayBoss(1487 + 200 * i, -1525, 30, false)
								for (let i = 0, len = (scale * 0.23) / 4; i < len; ++i)
									spawn.mineBoss(1487 + 200 * i, -1525, 50, false)
							} else {
								if (Math.random() < 0.25) {
									for (let i = 0, len = scale * 0.25; i < len; ++i)
										spawn.timeBoss(1487 + 200 * i, -1525, 80, false) //spawn 1-2 at difficulty 15
								} else if (Math.random() < 0.33) {
									for (let i = 0, len = scale * 0.1; i < len; ++i)
										spawn.bounceBoss(1487 + 200 * i, -1525, 80, false) //spawn 1-2 at difficulty 15
								} else if (Math.random() < 0.5) {
									for (let i = 0, len = scale * 0.16; i < len; ++i)
										spawn.sprayBoss(1487 + 200 * i, -1525, 30, false) //spawn 2-3 at difficulty 15
								} else {
									for (let i = 0, len = scale * 0.23; i < len; ++i)
										spawn.mineBoss(1487 + 200 * i, -1525, 50, false) //spawn 3-4 at difficulty 15
								}
							}
		...
		if (level.levelsCleared === 0) {
			...
			if (localSettings.levelsClearedLastGame < 3) {
				...
				if (!simulation.isCheating && localSettings.levelsClearedLastGame > 1) {
					...
					simulation.makeTextLog(
						`for (let i <span class='color-symbol'>=</span> 0; i <span class='color-symbol'><</span> localSettings.levelsClearedLastGame <span class='color-symbol'>/</span> 3; i<span class='color-symbol'>++</span>)`
					)
					simulation.makeTextLog(
						`{ powerUps.spawn(m.pos.x, m.pos.y, "tech") <em>//simulation superposition</em>}`
					)
	...
	reservoir() {
		...
		if (simulation.isHorizontalFlipped) {
				...
				if (isWaterfallFilling) {
					if (slime.height < 5500) {
						...
						if (!m.isBodiesAsleep) {
							...
							if (
								player.position.x > waterFallX &&
								player.position.x < waterFallX + waterFallWidth &&
								player.position.y < slime.height
							) {
								Matter.Body.setVelocity(player, {
									x: player.velocity.x,
			...
			if (Math.random() < 0.1) {
				...
				const r = 250
				const hexagon = `${r} 0   ${r * Math.cos(5.236)} ${r * Math.sin(5.236)}    ${r * Math.cos(4.189)} ${
					r * Math.sin(4.189)
				}     ${-r} 0     ${r * Math.cos(2.0944)} ${r * Math.sin(2.0944)}      ${r * Math.cos(1.0472)} ${
					r * Math.sin(1.0472)
				}  `
		...
		if (isLevelReversed === false) {
			...
			if (simulation.difficulty > 1) {
				if (Math.random() < 0.2) {
					...
					spawn.randomLevelBoss(2300, -1400, [
						'shooterBoss',
						'launcherBoss',
						'laserTargetingBoss',
						'spiderBoss',
	...
	house() {
		...
		if (simulation.difficulty > 3) {
			...
			if (Math.random() < 0.16) {
				...
				spawn.randomLevelBoss(3100, -1850, [
					'shooterBoss',
					'spiderBoss',
					'launcherBoss',
					'laserTargetingBoss',
	...
	perplex() {
		...
		if (simulation.difficulty > 3) {
			if (Math.random() < 0.5) {
				spawn.randomLevelBoss(450, -1350, [
					'shooterBoss',
					'launcherBoss',
					'laserTargetingBoss',
					'streamBoss',
				...
				spawn.randomLevelBoss(-300, -3200, [
					'shooterBoss',
					'launcherBoss',
					'laserTargetingBoss',
					'streamBoss',
	...
	coliseum() {
		...
		//Boss Spawning
		if (simulation.difficulty > 3) {
			spawn.randomLevelBoss(6000, 700, [
				'pulsarBoss',
				'laserTargetingBoss',
				'powerUpBoss',
				'bomberBoss',
	...
	vats() {
		...
		level.custom = () => {
			...
			if (simulation.cycle >= nextBlockSpawn && body.length < 100) {
				var len = body.length
				body[len] = Matter.Bodies.polygon(
					Math.floor(Math.random() * 1700) + 1050,
					100,
			...
			for (let i = 0, len = body.length; i < len; i++) {
				if (
					body[i].position.x > 700 &&
					body[i].position.x < 3200 &&
					body[i].position.y > 1200 &&
					...
					const slowY =
						body[i].velocity.y > 0
							? Math.max(0.3, 1 - 0.0015 * body[i].velocity.y * body[i].velocity.y)
							: Math.max(0.98, 1 - 0.001 * Math.abs(body[i].velocity.y)) //down : up
			...
			for (let i = 0, len = mob.length; i < len; ++i) {
				if (
					mob[i].position.x > 700 &&
					mob[i].position.x < 3200 &&
					mob[i].alive &&
					!mob[i].isShielded &&
					mob[i].position.y > 1200
				) {
		...
		function hangingNGON(x, y, s, height, stiffness, pin, properties) {
			...
			function compound3Parts(x1, y1, v1, x2, y2, v2, x3, y3, v3, properties) {
				...
				setTimeout(function () {
					...
					compoundParts.collisionFilter.mask =
						cat.body | cat.player | cat.bullet | cat.mob | cat.mobBullet | cat.map
				}, 1000)
			}
	...
	tunnel() {
			...
			if (
				...
				mob.reduce((a, i) => {
					return (
						a ||
						(Math.sqrt(
							(i.position.x - 3600) * (i.position.x - 3600) +
								(i.position.y + 3600) * (i.position.y + 3600)
						) < 20000 &&
							i.isDropPowerUp)
					)
				}, false) &&
		...
		level.customTopLayer = () => {
			...
			// Fire damage
			let isInRange = flames.reduce(
				(a, i) =>
					a ||
					Math.sqrt((m.pos.x - i[0]) * (m.pos.x - i[0]) + (m.pos.y + 90 - i[1]) * (m.pos.y + 90 - i[1])) < 50,
				false
			)
			...
			if (secretAnimTrans > 0) {
				...
				if (secretAnimTrans >= 42) {
					...
					ctx.fillStyle =
						(hasMob ? '#ff6644' : '#ffff44') + Math.floor((secretAnimTrans - 40) * 12.75).toString(16)
			...
			if (secretAnimTrans2 > 0) {
				...
				if (secretAnimTrans2 >= 42) {
					...
					ctx.fillStyle =
						(tech.totalCount < 25 ? (tech.totalCount < 10 ? '#ffff44' : '#22ff22') : '#ff6644') +
						Math.floor((secretAnimTrans2 - 40) * 12.75).toString(16)
					ctx.fillText(
						'Threat level: ' + (tech.totalCount < 25 ? (tech.totalCount < 10 ? 'Low' : 'Medium') : 'HIGH'),
						1560,
						-3790
					)
		...
		while (mobList1.length > 5 - Math.sqrt(simulation.difficulty * 2.5) && mobList1.length) {
			...
			spawn[['hopper', 'sneaker', 'striker'][Math.floor(Math.random() * 3)]](
				mobList1[rand][0],
				mobList1[rand][1],
				60 + Math.random() * 10
			)
		...
		function drawProject(startPos, endPos1, endPos2, tValue, tValueM) {
			...
			if (tValue >= (tValueM * 2) / 3) {
				ctx.fillStyle =
					'#0055aa' +
					('0' + Math.floor(((tValue - (tValueM * 2) / 3) * 6.25 * 60) / tValueM).toString(16)).slice(-2)
				ctx.strokeStyle =
					'#000033' +
					('0' + Math.floor(((tValue - (tValueM * 2) / 3) * 12.75 * 60) / tValueM).toString(16)).slice(-2)
	...
	islands() {
		...
		if (!isSpawnedBoss) {
			isSpawnedBoss = true
			if (Math.random() < 0.33) {
				for (let i = 0, len = Math.min(simulation.difficulty / 20, 6); i < len; ++i)
					spawn.bounceBoss(59025, -17325, 50, false)
			} else if (Math.random() < 0.5) {
				for (let i = 0, len = Math.min(simulation.difficulty / 9, 8); i < len; ++i)
					spawn.sprayBoss(59025, -17325, 50, false)
			} else {
				for (let i = 0, len = Math.min(simulation.difficulty / 6, 10); i < len; ++i)
	...
	temple() {
		...
		const Rect = (function () {
			...
			Rect.prototype.has = function ({ x, y }) {
				return (
					x >= this.pos.x && x <= this.pos.x + this.width && y >= this.pos.y && y <= this.pos.y + this.height
				)
			}
			Rect.prototype.hasLine = function (eq) {
				...
				return (
					(leftInter >= this.pos.y && leftInter <= this.pos.y + this.height) ||
				...
				if (Math.random() > 0.3)
					powerUps.spawn(
						...
						30 *
							...
							Math.sqrt(tech.largerHeals) *
							Math.sqrt(0.1 + Math.random() * 0.5)
					)
			}
			me.damageReduction = 0.25 / (tech.isScaleMobsWithDuplication ? 1 + tech.duplicationChance() : 1)
			me.do = function () {
				...
				if (this.seePlayer.recall) {
					...
					if (distance(this.position, player.position) < eventHorizon) {
						if (this.isDark) {
							...
							ctx.fillStyle = `rgba(255,240,127,${
								0.12 * Math.max(15 - simulation.cycle + this.lastAttackCycle, 0)
							})`
							DrawTools.arc(m.pos.x, m.pos.y, 40, 0, 2 * Math.PI)
						}
					}
		...
		function thirdRoomBoss(x, y) {
			...
			me.ring = function () {
				...
				for (const ring of rings) {
					...
					if (
						Math.abs(distance(player.position, this.position) - radius) < 60 &&
						m.immuneCycle < simulation.cycle
					) {
						m.damage(0.4 / radius)
					}
				...
				if (!(this.spawnCycle % 300)) {
					for (let i = 0; i < 3; i++) {
						mobGrenade(
							...
							-13030,
							undefined,
							Math.min(700, 300 + simulation.difficulty * 4),
							10
						)
					...
					if (!(cycle % spawnDelay)) {
						...
						if (Math.floor(cycle / spawnDelay) >= spawnCycles - 1) {
							...
							this.damageReduction =
								0.25 / (tech.isScaleMobsWithDuplication ? 1 + tech.duplicationChance() : 1)
						}
					}
				}
			...
			for (let i = 0, len = powerUpCollide.length; i < len; i++) {
				...
				const buoyancy =
					...
					(0.2 *
						Math.max(
							0,
							Math.min(diameter, this.min.y - powerUpCollide[i].position.y + powerUpCollide[i].size)
						)) /
						diameter
				powerUpCollide[i].force.y -= buoyancy * 1.14 * powerUpCollide[i].mass * simulation.g
		...
		const Objects = {
			altar: {
				...
				get isActive() {
					...
					return (
						distance(roughPlayerCentre, this.pos) < 240 &&
					...
					Promise.resolve().then(() => {
						...
						if (!this.capped && this.distance < 400) {
							ctx.fillStyle = `rgba(0, 0, 0, ${
								Math.min(1, (400 - this.distance) / (400 - this.range)) * 0.9
							})`
						...
						if (this.distance < this.range && !this.capped) {
							DrawTools.lightning(
								V.sub(this.pos, { x: 300, y: 300 }),
								V.add(this.pos, { x: 300, y: 300 }),
								simulation.cycle - 5
							)
							DrawTools.lightning(
								V.add(this.pos, { x: -300, y: 300 }),
						...
						if (templePlayer.room2.cycles <= 100) {
							for (let i = 0; i < this.ringNumber; i++) {
								...
								const opacity = this.capped
									? 1 - 0.01 * templePlayer.room2.cycles
									: cycle / 180 - i / 2 - 0.5
								ctx.strokeStyle = `rgba(${ring.colour.join(',')}, ${Math.min(opacity, 1)})`
								const radius =
									(this.capped
										? 1 + 0.07 * templePlayer.room2.cycles
										: Math.sin((Math.min(cycle - i * 90 - 90, 45) / 90) * Math.PI)) * ring.radius
			...
			room2GeneratedPath: {
				...
				logic() {
					if (
						templePlayer.room2.readyPathCycle &&
						simulation.cycle - templePlayer.room2.readyPathCycle === 180
					) {
				...
				draw() {
					if (
						...
						for (const r of this.rects) {
							DrawTools.lightning(
								Objects.room2Initiator.pos,
								r.midPos,
								templePlayer.room2.readyPathCycle + 180
							)
						}
					}
				}
		...
		const RoomTransitionHandler = {
			...
			room1() {
				...
				if (templePlayer.room1ToRoom2Anim === 1260) {
					...
					const x = ['a speck of dust', 'an insignificant hindrance', 'a tiny obstacle'][
			...
			room2() {
				...
				if (templePlayer.room2ToRoom3Anim > 660 && templePlayer.room2ToRoom3Anim <= 840) {
					canvas.style.filter = `sepia(${(templePlayer.room2ToRoom3Anim - 660) / 180}) invert(${
						0.5 + (templePlayer.room2ToRoom3Anim - 660) / 360
					})`
				}
				...
				if (templePlayer.room2ToRoom3Anim > 960 && templePlayer.room2ToRoom3Anim <= 1140) {
					canvas.style.filter = `sepia(${(1140 - templePlayer.room2ToRoom3Anim) / 180}) invert(${
						(1140 - templePlayer.room2ToRoom3Anim) / 180
					})`
				}
			...
			room3() {
				...
				for (let i = 0; i < 12; i++) {
					if (templePlayer.room3ToEndAnim === 720 + i * 20) {
						name = name.slice(0, -1)
						simulation.makeTextLog(
							`<span style="font-size: 1em"><span style="color: #f00">${name}:</span> &nbsp; ${
								text[i % 6]
							}</span>`
						)
				...
				if (!isInBounds) {
					...
					simulation.makeTextLog(
						"<span style='color: #f00'>" +
							name +
							'</span>: &nbsp; You thought I could let you get away with that?'
					)
				}
			},
			room0() {
		...
		const DrawHandler = {
			...
			room1() {
				...
				const spawnFlameAngle =
					(Math.min(Math.min(templePlayer.room1.cycles, 2520) % 600, 120) * Math.PI) / 30 + Math.PI / 2
			...
			room2() {
				...
				if (templePlayer.room2.cycles) {
					ctx.fillStyle = '#0006'
					ctx.fillRect(
						secondRoomBounds.pos.x + 2,
						secondRoomBounds.pos.y + 2,
						secondRoomBounds.width - 4,
						secondRoomBounds.height - 4
					)
				}
				room2SlimePit.draw()
			},
			room3() {
				...
				ctx.fillStyle = '#0006'
				ctx.fillRect(
					thirdRoomBounds.pos.x + 2,
					thirdRoomBounds.pos.y + 2,
					thirdRoomBounds.width - 4,
					thirdRoomBounds.height - 4
				)
		...
		const WaveHandler = {
			room1() {
				...
				if (templePlayer.room1.cycles % 600 === 0 && templePlayer.room1.cycles <= 2400) {
					const spawnAmt =
						Math.min(3 + Math.pow(simulation.difficulty / 1.7, 0.6), 10) +
						Math.floor(templePlayer.room1.cycles / 720)
		...
		const DrawTools = {
			...
			updateRandomColours(x = 0.8) {
				for (let i = 0; i < this._randomColours.length; i++) {
					this._randomColours[i] = Math.max(
						Math.min(this._randomColours[i] + this.randFact() * x * 2 - x, 255),
						0
					)
				}
			},
			...
			lightning(from, to, cycle, randomPRNGMult = 1, width = 8, color = [255, 240, 127]) {
				...
				while (1) {
					const randOffset = rotateVector(
						{ y: RNG(Math.floor(cycle * randomPRNGMult + distanceLeft)) * 2 * d - d, x: 0 },
						normalised
					)
		...
		function trapPlayer(x, y) {
			...
			for (let i = 0; i < bLen; i++) {
				if (bullet[i].botType)
					setPosAndFreeze(
						bullet[i],
						V.add(player.position, { x: 100 * (RNG(i) - 0.5), y: 100 * (RNG(i + bLen) - 0.5) })
					)
			}
		}
		...
		const makeLore = x =>
			simulation.makeTextLog(
				`<h2 style='color: #f00; display: inline-block'>${name}:</h2> &nbsp; <h3 style='display: inline-block'>${x}</h3>`
			)
	...
	walk() {
		...
		let instruction = 0
		level.trainingText(
			`move <strong>↔</strong> with <strong class="key-input-train">${input.key.left
				.replace('Key', '')
				.replace('Digit', '')}</strong> and <strong class="key-input-train">${input.key.right
				.replace('Key', '')
				.replace('Digit', '')}</strong>`
		)

		level.custom = () => {
			if (instruction === 0 && input.right) {
				instruction++
				level.trainingText(`<s>move <strong>↔</strong> with <strong class="key-input-train">${input.key.left
					.replace('Key', '')
					.replace('Digit', '')}</strong> and <strong class="key-input-train">${input.key.right
					.replace('Key', '')
					.replace('Digit', '')}</strong></s>
	...
	crouch() {
		...
		let instruction = 0
		level.trainingText(
			`press <strong class="key-input-train">${input.key.down
				.replace('Key', '')
				.replace('Digit', '')}</strong> to crouch`
		)
		level.custom = () => {
			if (instruction === 0 && input.down) {
				instruction++
				level.trainingText(
					`<s>press <strong class="key-input-train">${input.key.down
						.replace('Key', '')
						.replace('Digit', '')}</strong> to crouch</s>`
				)
			}
			//exit room
			ctx.fillStyle = '#f2f2f2'
	...
	jump() {
		...
		let instruction = 0
		level.trainingText(
			`hold down <strong class="key-input-train">${input.key.up
				.replace('Key', '')
				.replace('Digit', '')}</strong> longer to jump higher`
		)
		...
		level.custom = () => {
			if (instruction === 0 && m.pos.x > 300) {
				instruction++
				level.trainingText(
					`<s>hold down <strong class="key-input-train">${input.key.up
						.replace('Key', '')
						.replace('Digit', '')}</strong> longer to jump higher</s>`
				)
			}
	...
	hold() {
		...
		let instruction = 0
		level.trainingText(
			`activate your <strong class='color-f'>field</strong> with <strong class="key-input-train">${input.key.field
				.replace('Key', '')
				.replace('Digit', '')}</strong> or <strong>right mouse</strong>`
		)
		...
		level.custom = () => {
			if (instruction === 0 && input.field) {
				instruction++
				level.trainingText(
					`<s>activate your <strong class='color-f'>field</strong> with <strong class="key-input-train">${input.key.field
						.replace('Key', '')
						.replace(
							'Digit',
							''
						)}</strong> or <strong>right mouse</strong></s><br>release your <strong class='color-f'>field</strong> on a <strong class='color-block'>block</strong> to pick it up`
				)
				...
				level.trainingText(
					`<s>activate your <strong class='color-f'>field</strong> with <strong class="key-input-train">${input.key.field
						...
						.replace(
							'Digit',
							''
						)}</strong> or <strong>right mouse</strong><br>release your <strong class='color-f'>field</strong> on a <strong class='color-block'>block</strong> to pick it up</s><br>drop the <strong class='color-block'>block</strong> on the red button to open the door`
				)
			} else if (
				instruction === 2 &&
				!buttonDoor.isUp &&
				Vector.magnitudeSquared(Vector.sub(body[0].position, buttonDoor.min)) < 10000
			) {
				instruction++
				level.trainingText(
					`<s>activate your <strong class='color-f'>field</strong> with <strong class="key-input-train">${input.key.field
						...
						.replace(
							'Digit',
							''
						)}</strong> or <strong>right mouse</strong><br>release your <strong class='color-f'>field</strong> on a <strong class='color-block'>block</strong> to pick it up<br>drop the <strong class='color-block'>block</strong> on the red button to open the door</s>`
				)
			}
			//exit room
			ctx.fillStyle = '#f2f2f2'
	...
	throw() {
		...
		let instruction = 0
		level.trainingText(
			`pick up the <strong class='color-block'>block</strong> with your <strong class='color-f'>field</strong>`
		)
	...
	throwAt() {
		...
		let instruction = 0
		level.trainingText(
			`throw the <strong class='color-block'>block</strong> at the <strong>mobs</strong> to open the door`
		)
		...
		level.custom = () => {
			if (instruction === 0 && !mob.length) {
				instruction++
				level.trainingText(
					`<s>throw the <strong class='color-block'>block</strong> at the <strong>mobs</strong> to open the door</s>`
				)
			}
			//exit room
			ctx.fillStyle = '#f2f2f2'
	...
	fire() {
		...
		level.custom = () => {
			...
			if (!powerUp.length) {
				...
				if (b.inventory.length && b.guns[b.activeGun].ammo === 0)
	...
	deflect() {
		...
		level.trainingText(
			`use your <strong class='color-f'>field</strong> to <strong>deflect</strong> the <strong style="color:rgb(215,0,145);">mobs</strong>`
		)
		...
		level.custom = () => {
			if (instruction === 0 && m.pos.x > 1350) {
				instruction++
				level.trainingText(
					`<s>use your <strong class='color-f'>field</strong> to <strong>deflect</strong> the <strong style="color:rgb(215,0,145);">mobs</strong></s>`
				)
			}
	...
	heal() {
		...
		level.custom = () => {
			if (instruction === 0 && m.health === 1) {
				instruction++
				level.trainingText(
					`<s>use your <strong class='color-f'>field</strong> to pick up <div class="heal-circle" style = "border: none;"></div> until your <strong>health</strong> is full</s>`
				)
			}
			//exit room
			ctx.fillStyle = '#f2f2f2'
	...
	nailGun() {
		...
		let instruction = 0
		level.trainingText(
			`use your <strong class='color-f'>field</strong> to pick up <div class="ammo-circle" style = "border: none;"></div> for your <strong class='color-g'>nail gun</strong>`
		)
	...
	missile() {
		...
		let instruction = 0
		level.trainingText(
			`use <strong class='color-g'>missiles</strong> to drop a <strong class='color-block'>block</strong> on the button`
		)
		...
		level.custom = () => {
			if (instruction === 0 && mob.length === 0) {
				instruction++
				level.trainingText(
					`<s>use <strong class='color-g'>missiles</strong> to drop a <strong class='color-block'>block</strong> on the button</s>`
				)
			}
			//spawn ammo if you run out
			let isAmmo = false
	...
	stack() {
		...
		let instruction = 0
		level.trainingText(
			`use your <strong class='color-f'>field</strong> to stack the <strong class='color-block'>blocks</strong>`
		)
		...
		level.custom = () => {
			if (instruction === 0 && m.pos.x > 1635) {
				instruction++
				level.trainingText(
					`<s>use your <strong class='color-f'>field</strong> to stack the <strong class='color-block'>blocks</strong></s>`
				)
			}
	...
	mine() {
		...
		level.custom = () => {
			if (instruction === 0 && !button.isUp) {
				instruction++
				level.trainingText(
					`<s>press the red <strong>button</strong> to spawn a <strong>mob</strong></s><br>turn the <strong>mobs</strong> into <strong class='color-block'>blocks</strong>`
				)
			} else if (instruction === 1 && body.length > 2) {
				instruction++
				level.trainingText(
					`<s>press the red <strong>button</strong> to spawn a <strong>mob</strong><br>turn the <strong>mobs</strong> into <strong class='color-block'>blocks</strong></s><br>use your <strong class='color-f'>field</strong> to stack the <strong class='color-block'>blocks</strong>`
				)
			} else if (instruction === 2 && m.pos.x > 1635) {
				instruction++
				level.trainingText(
					`<s>press the red <strong>button</strong> to spawn a <strong>mob</strong><br>turn the <strong>mobs</strong> into <strong class='color-block'>blocks</strong><br>use your <strong class='color-f'>field</strong> to stack the <strong class='color-block'>blocks</strong></s>`
				)
			}
			//spawn ammo if you run out
			let isAmmo = false
	...
	grenades() {
		...
		level.custom = () => {
			if (instruction === 0 && elevator1.isOn) {
				...
				instruction++
				level.trainingText(
					`<s>flip the <strong>switch</strong> to turn on the <strong>elevator</strong><br>put a <strong class='color-block'>block</strong> on the <strong>button</strong> to active the <strong>elevator</strong><br>hold <strong>jump</strong> before the <strong>elevator's</strong> <strong>apex</strong> to reach the <strong>exit</strong></s>`
				)
			}
			//exit room
			ctx.fillStyle = '#f2f2f2'
