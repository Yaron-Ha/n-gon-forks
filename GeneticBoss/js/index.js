...
document.body.addEventListener("mouseup", (e) => {
    if (e.button === 0) {
        // 0 -> left mouse button, used for firing
        input.fire = false;
    } else if (e.button === 2) {
        // 2 -> right mouse button, used for field
        input.field = false;
    } else if (e.button === 4 && simulation.isConstructionMode) {
        // 4 -> forward button, an extra button on the mouse. If it's clicked
        // and construction mode is enabled, then copy the player's coords.
        // ~~ -> quick truncation/floor
        simulation.outputMapString(`${~~(simulation.mouseInGame.x)}, ${~~(simulation.mouseInGame.y)}, `)
