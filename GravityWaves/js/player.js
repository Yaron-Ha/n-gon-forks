                ...
                m.hold = function() {
                    ...
                    if (input.field) {
                        if (m.fieldCDcycle < m.cycle) {
                            ...
                            if (tech.isGravityWaves) {
                                // boom
                                for (let i = 0, len = mob.length; i < len; ++i) {
                                    const dxP = m.fieldPosition.x - mob[i].position.x;
                                    const dyP = m.fieldPosition.y - mob[i].position.y;
                                    ...
                                    if (
                                        ...
                                        (m.lookingAt(mob[i]) || dist2 < 16000)
                                    ) {
                                        if (!m.suckedMobs.includes(mob[i])) {
                                            // make them not collide with each other
                                            mob[i].collisionFilter =  {
                                                group: 0,
                                                category: cat.mob,
                                                mask: cat.player | cat.map | cat.body
                                            }
                                            m.suckedMobs.push(mob[i])
                                        }

                                        mob[i].force.x += 0.01 * (dxP / Math.sqrt(dist2)) * mob[i].mass;
                                        mob[i].force.y += 0.01 * (dyP / Math.sqrt(dist2)) * mob[i].mass - mob[i].mass * simulation.g; //negate gravity
                                        //extra friction
                                        Matter.Body.setVelocity(mob[i], {
                                            x: mob[i].velocity.x * 0.11,
                                            y: mob[i].velocity.y * 0.11
                                        })

                                        if (dist2 < 1000) {
                                            m.massAtCenter += mob[i].mass
                                        }
                                    }
                                }
                            } else {
                                //grab power ups into the field, if it's a regular pilot wave
                            ...
                            if (m.energy > 0.01) {
                                ...
                                for (let i = 0, len = body.length; i < len; ++i) {
                                    if (Vector.magnitude(Vector.sub(body[i].position, m.fieldPosition)) < m.fieldRadius && !body[i].isNotHoldable) {
                                        const DRAIN = speed * body[i].mass * (tech.isGravityWaves ? 0.000012 : 0.000006) // * (1 + m.energy * m.energy) //drain more energy when you have more energy
                                        if (m.energy > DRAIN) {
                                            m.energy -= DRAIN;
                                ...
                                if (tech.isGravityWaves) {
                                    if (m.massAtCenter > CRITICAL_MASS) {
                                        ctx.fillStyle = "rgba(255, 100, 100, 0.5)"
                                    } else {
                                        const num = 255 - ~~m.massAtCenter * 3
                                        ctx.fillStyle = `rgba(${num},${num},${num},0.5)`
                                    }
                                } else {
                        ...
                        m.fieldOn = false
                        m.fieldRadius = 0
                        // reset mob collisions
                        while (m.suckedMobs.length > 0) {
                            const next = m.suckedMobs.pop()
                        ...
                        if (m.massAtCenter > CRITICAL_MASS) {
                            b.explosion(m.fieldPosition, m.massAtCenter / 2)
                            while (m.suckedMobs.length > 0) {
                                const next = m.suckedMobs.pop()
                                next.leaveBody = false
                                next.death()
                            }
                        }
