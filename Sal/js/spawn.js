...
const spawn = {
    ...
    snakeBoss(x, y, radius = 50, isBaby = false) { //snake boss with a laser head
        const nodes = isBaby ? 3 : Math.min(20 + simulation.difficulty, 60)
        let angle = Math.PI
        let mag = 300
        ...
        me.onDeath = function() {
            powerUps.spawnBossPowerUp(this.position.x, this.position.y)
            for (let i = 0; i < mob.length; i++) { //wake up tail mobs
                if (mob[i].isSnakeTail && mob[i].alive && mob[i].father == this) {
                    mob[i].isSnakeTail = false;
                    mob[i].do = mob[i].doActive
                    // mob[i].removeConsBB();
                }
        ...
        me.do = function() {
            ...
            if (this.seePlayer.recall && !this.isBaby && Math.random() > 0.995) {
                // yeet last node at player
                const lastNode = me.nodeList.pop()

                lastNode.kamikazeTicks = 200 + ~~(Math.random() * 30)
                lastNode.stroke = "#000"
                lastNode.fill = "#4a4a4a"
    ...
    snakeBody(x, y, radius = 10) {
        ...
        me.do = function() {
            this.checkStatus()
            if (this.kamikazeTicks >= 0) {
                this.alwaysSeePlayer()
                if (this.kamikazeTicks === 0) {
                    // remove cons when reaching 0
                    this.removeConsBB()
                }
                this.attraction()
                this.kamikazeTicks--
