...
const b = {
    ...
    nuke(pos) {
        ...
        const MAX_PRIMING_TIME = 500
        const PRIMING_HEIGHT = 5000

        // spawn it pointing up
        b.missile(pos, -Math.PI / 2, 0, ROCKET_SIZE)
        const me = bullet[bullet.length - 1]
        // modify the regular rocket to be a nuke!
        // give it a direct upwards motion
        me.velocity = {x: 0, y: 1}
        me.endCycle = Infinity
        // nukes arne't homing
        me.tryToLockOn = () => {}
        // remove default collision explosion
        me.onEnd = () => {}
        me.beforeDmg = () => {}
        // the mass determines the speed, so the mass starts high and gets lower
        // for a cool acceleration effect
        me.mass = 10000
        // the nuke gets MAX_PRIMING_TIME ticks to reach PRIMING_HEIGHT.
        // when it reaches that height and the time passes, it'll nuke. Otherwise,
        // it'll simply explode powerfully.
        me.tick = 0
        // update the do by taking the old do and running it after new code.
        // the bind is important, because the moment `do` is detached it loses its old reference to `this`.
        const oldDo = me.do.bind(me)
        // used to determine if the rocket should stop. Will reserve computing power when priming
        me.shouldFreeze = false
        // keeps track on the nuking part itself
        // function instead of a lambda since I need a `this` reference.
        me.do = function() {
            // for acceleration
            if (this.mass > 100) {
                this.mass -= 40
            ...
            // when the priming time is complete, return if the height isn't enough
            if (this.tick == MAX_PRIMING_TIME) {
                this.shouldFreeze = true
                // simple explode if the height is too low
                if (-this.position.y < PRIMING_HEIGHT) {
                    b.explosion(this.position, 2500)
                    this.endCycle = 0
                    simulation.makeTextLog(`The nuke was <strong class='color-d'>primed</strong> but didn't reach the correct height in order to create a <strong class='color-p'>nuclear detonation</strong>.`)
                    // reset shake
                    canvas.classList = []
                    return
                } else {
                    // nobody will survive a nuke once it's launched
                    level.nextLevel = () => {
                        // hex looks very codey
                        simulation.makeTextLog(`Error: high <strong class='color-p'>radiation</strong> levels detected. Transportation to the next level <strong class='color-r'>failed</strong>: error code 0x${Math.round(Math.random() * 10000000).toString(16).toUpperCase()}.`)
                    }
                }
            }

            // otherwise, run this code that controls the explosion sequence
            if (++this.tick > MAX_PRIMING_TIME) {
                const nukeTick = this.tick - MAX_PRIMING_TIME
                // time special behavior with nukeTick.
                // things that happen only once in a point in time
                switch (nukeTick) {
                    case 50:
                        simulation.makeTextLog("<strong class='color-p'>Oh no...</strong>")
                        break
                    case 100:
                        simulation.wipe = function() {
                            // I'm inlining nukeTick since it doesn't work here (stays constant)
                            const diff = Math.max(0, 255 - ~~((me.tick - MAX_PRIMING_TIME)/ 10)) // can't go below 0
                            ctx.fillStyle = `rgba(255,${diff},${diff},${me.tick * 0.001})`
                            ctx.fillRect(0, 0, canvas.width, canvas.height)
                        }
                        break
                    case 1500:
                        // ok, time to return the missile...
                        this.mass = 2000
                        Matter.Body.setPosition(this, Vector.add(m.pos, {x: 0, y: -10000}))
                        Matter.Body.rotate(this, Math.PI) // should be more at the player's direction
                        this.shouldFreeze = false // unfreeze
                        this.lockedOn = player // oh no...
                }

                // other things repeat.
                // kill all mobs slowly
                if (nukeTick > 600) {
                    for (let entity of mob) {
                        // all mobs will eventually die
                        if (Math.random() > 0.99 && !entity.nukeResistant) {
                            entity.death()
                        }
                    }
                }
                // shake all blocks with growing intensity
                if (nukeTick > 800) {
                    for (let entity of body) {
                        const angle = Math.random() * 2 * Math.PI // random angle
                        let vector = {x: Math.cos(angle), y: Math.sin(angle)} // convert to vector
                        vector = Vector.mult(vector, (nukeTick - 800) / 25) // apply some strength
                        Matter.Body.setVelocity(entity, vector)
                    }
                }
                // ok enough block shaking, yeet the blocks
                if (nukeTick > 1200) {
                    for (let i = 0; i < body.length; i++) {
                        const entity = body[i];
                        // destroy block
                        if (Math.random() > 0.995) {
                            b.explosion(entity.position, entity.mass)
                            Matter.Composite.remove(engine.world, entity)
                            body.splice(i, 1)
                            // spawn harmful bullets
                            for (let i = 0; i < Math.random() * 10; i++) {
                                spawn.bullet(entity.position.x, entity.position.y, Math.random() * 30)
                                const thatBullet = mob[mob.length - 1]
                                const angle = Math.random() * 2 * Math.PI // random angle
                                let vector = {x: Math.cos(angle), y: Math.sin(angle)} // convert to vector
                                vector = Vector.mult(vector, 25 * Math.random()) // apply some strength
                                Matter.Body.setVelocity(thatBullet, vector)
                                // a market that makes the loop above not instant kill it
                                thatBullet.nukeResistant = true
                            }
                        }
                    }
                }
                // now let's make it land!
                if (nukeTick > 1500) {
                    if (Matter.Query.collides(this, map).length > 0) {
                        // touched map... Boom
                        b.explosion(this.position, 378042)
                        this.endCycle = 0
                        // do a sound effect
                        boomSound.play()
                    }
            ...
            // make some interesting logs
            if (this.tick < MAX_PRIMING_TIME && this.tick % 100 == 0) {
                simulation.makeTextLog(`Nuke will <strong class='color-d'>prime</strong> in ${MAX_PRIMING_TIME - this.tick} ticks!`)
            }

            const dist = Vector.magnitude(Vector.sub(this.position, m.pos))

            // if the player is close enough to the rocket or the rocket is nuking, shake the canvas
            if (dist < 2000 || this.tick > MAX_PRIMING_TIME) {
                canvas.classList = ['shake']
            } else {
                canvas.classList = []
            }
                        
            // call the regular missile activity
            if (!this.shouldFreeze) oldDo()
        }
    },
