const tech = {
    ...
    tech: [{
        ...
        {
            name: "Tipoos",
            descriptionFunction() { return `Cheygnes ol neyms end deskreepshens tu bi fool of tipoos` },
            maxCount: 1,
            count: 0,
            frequency: 0,
            isJunk: true,
            ...
            effect() {
                function dumbify(input) {
                    // I assume this is possible with Regex, but I don't really know how to use it.
                    // replace table, built in the following fashion:
                    // [input, output, verifierFunction(index) => bool?, fallBack?]
                    // if the verifier function returns true, then the fallback is selected instead
                    // the fallback must be defined if the verifier function is.
                    const rTable = [
                        // priority matters.
                        // ** LONG WORDS **
                        ['spawn', 'spon'],
                        ...
                        ['damage', 'demej'],
                        ['JUNK', 'JOHNQUE'],
                        ['power', 'pawer'], // as in "power up"
                        // ** SHORT WORDS AND LONG HOMOPHONES **
                        // these are sorrounded with space to make sure words like "tomato" don't become "tumatu"
                        [' to ', ' too '],
                        [' a ', ' eh '],
                        [' the ', ' de '],
                        ['die', 'dai'],
                        ...
                        ['ck', 'que'],
                        // ** SHORT HOMOPHONES **
                        // these replacement don't always stay homophones
                        // always replace c with k, unless the next letter is affected by that
                        ['c', 'k', i => ['e','i','h'].includes(input[i + 1]), 'c'],
                        // replace k with c, unless the next letter is e or i, otherwise use q
                        ['k', 'c', i => ['e','i','y'].includes(input[i + 1]), 'q'],
                        ['q', 'k'],
                        // change all u to oo, unless the letter before is also o (otherwise unreadable, trust me)
                        ['u', 'oo', i => input[i - 1] && input[i - 1] == 'o', 'u'],
                        ['oo', 'u'],
                        ['y', 'i'], ['ee', 'i'], ['ea', 'i'],
                        ['x', 'cs'],
                        // remove silent e if it's the end of the world (indicated by the next
                        // letter being a space, undefined (end of string) or a tag start)
                        ['e', 'e', i => [' ','<',undefined].includes(input[i + 1]), ''],
                        // make a terminating s funny
                        ['s', 's', i => [' ','<',undefined].includes(input[i + 1]), 'z'],
                    ...
                    while (i < input.length) {
                        // using a while rather than a far because it allows me to jump more than one at once
                        const token = input[i]
                        if (token == '<') isInsideHtml = true
                        else if (token == '>') isInsideHtml = false
                        // don't do anything inside HTML tags
                        if (isInsideHtml) {
                            buffer.push(token)
                            i++
                            continue
                        ...
                        // the result token(s)
                        let result
                        // check if the rTable has an entry for this letter
                        let tableEntry
                        for (const replacer of rTable) {
                            // check if any replacer matches the current token(s) at this index
                            const replacerInput = replacer[0]
                            // try to form a substring of the same length. If the string is too short, skip
                            if (i + replacerInput.length > input.length) {
                                continue
                            }
                            // substring, apply heurostic if the length is 1
                            const subs =
                                replacerInput.length == 1 ? input[i] : input.substring(i, i + replacerInput.length)
                            if (subs == replacerInput) {
                                // if the substring described is the as the input needed for the replacer,
                                // choose that replacer
                                tableEntry = replacer
                                break
                            }
                        }
                        ...
                        if (tableEntry) {
                            // if so, check if the predicate exists. Push the fallback if the
                            // predicate isn't defined or it returns false, otherwise push the replacement value
                            const predicate = tableEntry[2]
                            if (!predicate || !predicate(i)) {
                                // predicate does not apply, choose the output char
                                result = tableEntry[1]
                            } else {
                                // predicate does apply, choose the fallback
                                result = tableEntry[3]
                            }
                        } else {
                            // if there is no rTable just map it verbatim
                            result = token
                        }
                        // push the result, and progress i by the amount of tokens consumed
                        // console.log(result)
                        buffer.push(result)
                        i += tableEntry ? tableEntry[0].length : 1
                    }
                    // add the buffer, stringified
                    return buffer.join('')
                }
                // takes a list of elements that contain a description field
                function awfulize(list) {
                    for (const choice of list) {
                        // dumbify description or description function
                        if (choice.descriptionFunction) {
                            // captuore the old function and update it
                            const fun = choice.descriptionFunction
                            const wrapperFunction = function() { return dumbify(fun()) }
                            choice.descriptionFunction = wrapperFunction
                        } else {
                            // no function, just a string
                            choice.description = dumbify(choice.description)
                        }
                    }
                }
                // make all techs awful
                awfulize(tech.tech)
                // make all guns awful
                awfulize(b.guns)
                // make all fields awful
                awfulize(m.fieldUpgrades)
                // make text log awful
                const old = simulation.makeTextLog
                simulation.makeTextLog = function(text, time = 240) {
                    const newText = dumbify(text)
                    old(newText, time)
                }
                // ... make lore awful
                function awfulizeLorePerson(person) {
                    const old = person.text
                    person.text = function(say) {
                        const newSay = dumbify(say)
                        old(newSay)
                    }
                }
                awfulizeLorePerson(lore.trainer)
                awfulizeLorePerson(lore.anand)
                awfulizeLorePerson(lore.miriam)
        ...
        },
        {
            ...
            description: "Decay information in tech descriptions",
            maxCount: Infinity,
            count: 0,
            isJunk: true,
            frequency: 1,
            frequencyDefault: 1,
            isNonRefundable: true,
            isBadRandomOption: false,
            allowed(){
                return true;
            },
            requires: "",
            effect() { /* WARNING: If you're still using the bad research orb generator, this tech doesn't work. */
                for (const techEntry of tech.tech) {
                    const mangler = (entryString) => {
                        // message split into tokens
                        const tokens = /* you're welcome Shawarma */ entryString.split(/ (?=[^>]*(?:<|$))/)
                        // that token list but purged and cleaned
                        const purged = []
                        for (let i = 0; i < tokens.length; i ++){
                            if (tokens[i] == "&nbsp;") {
                                purged[purged.length - 1] += "&nbsp; "
                            }
                            else if (tokens[i].length > 0) { // also remove empty strings
                                purged.push(tokens[i] + " ")
                            }
                        }
                        // shuffle and return
                        return shuffle(purged).join("")
                    }
                    
                    if (techEntry.description) {
                        // if it uses a plain description, just modify it directly
                        techEntry.description = mangler(techEntry.description)
                    } else {
                        // if it uses a special function, wrap it in another function
                        const originFunction = techEntry.descriptionFunction
                        techEntry.descriptionFunction = () => mangler(originFunction())
                    }
                }
            },
            remove(){
